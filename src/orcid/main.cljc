(ns orcid.main
  "Kotodama WASM entrypoint for the Orcid clean-room actor (L4) — Clojure port.
  ...
  facts (`orcid.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Work"      :plural "works"      :id-prefix "orcid_wor"
    :fields [:recordId :putCode :title :subtitle :translatedTitle :journalTitle :shortDescription :citationType :citationValue :type :publicationDate :url :languageCode :country :visibility] :required [:title]
    :coerce {:putCode :int}                   :refs {:recordId "Record"}}
   {:entity "Author"    :plural "authors"    :id-prefix "orcid_aut"
    :fields [:orcid :givenName :familyName]  :required [:givenName]
    :coerce {}                                :refs {}}
   {:entity "Citation"  :plural "citations"  :id-prefix "orcid_cit"
    :fields [:workId :citedDoi]               :required [:citedDoi]
    :coerce {}                                :refs {:workId "Work"}}
   {:entity "Standard"  :plural "standards"  :id-prefix "orcid_sta"
    :fields [:designation :title :organization :status] :required [:designation :title]
    :coerce {}                                :refs {}}
   {:entity "Record"    :plural "records"    :id-prefix "orcid_rec"
    :fields [:accession :database :title]    :required [:accession :database]
    :coerce {}                                :refs {}}
   {:entity "Subject"   :plural "subjects"   :id-prefix "orcid_sub"
...
(defn emit-facts
  "EAVT facts for one record: {\"orcid.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "orcid-compat" :tier tier :entities entities} 200])
